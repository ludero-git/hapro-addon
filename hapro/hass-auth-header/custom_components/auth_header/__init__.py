...
async def async_setup(hass: HomeAssistant, config):
    ...
    # Inject Auth-Header provider after trusted_networks.
    ...
    for key, existing_provider in hass.auth._providers.items():
        new_providers[key] = existing_provider
        if getattr(existing_provider, "type", None) == "trusted_networks":
            new_providers[(provider.type, provider.id)] = provider
            inserted = True
            
    if not inserted:
        # Fallback: inject at the top if trusted_networks is missing
        new_providers = OrderedDict()
        new_providers[(provider.type, provider.id)] = provider
        new_providers.update(hass.auth._providers)

    hass.auth._providers = new_providers
